...
export default class UnmuteCommand extends SlashCommand {
    ...
    async run({ bot, interaction, serverConf, verifiedConfig: userConfig }: SlashCommandContext): Promise<void | Message<boolean> | InteractionResponse> {
        ...
        if ((userconfig.mute?.end || 0) < Date.now()) return interaction.reply({ content: `**${user?.username}** is not muted.`, ephemeral: true });
        userconfig.moderate().unmute(interaction.user.id, reason);
        return bot.createEmbed(interaction)
            .setColor('Green')
            .setContent(`<@${user.id}>`)
            .setFooter({ text: `Punishment lifted by ${interaction.user.username}` }).setTimestamp()
            .setDescription(`**${user?.username}** has been **unmuted** for *${reason}*`).send();
    }
}