...
export default class NukeGameChannelsCommand extends SlashCommand {
    constructor() {
        super({
            name: "nukegamechannels",
            adminOnly: true,
            slash: new SlashCommandBuilder()
                .setName("nukegamechannels")
                .setDescription("nuke game channels")
        });
    }
    async run({ bot, interaction, serverConf, verifiedConfig: userConfig }: SlashCommandContext): Promise<void | Message<boolean> | InteractionResponse> {
        const gamesCategory = await interaction.guild.channels.fetch(bot.config.channels.gamesCategory) as CategoryChannel;
        interaction.reply({ embeds: [bot.createSuccessEmbed().setDescription(`nuking channels...`)], ephemeral: true });
        gamesCategory.children.cache.forEach(async channel => {
